...
if (isset($_POST['update'])) {
    ...
    $notice_desc=$_POST['desc'];
    $event_id=$_POST['eventid']; 
    $updated_file = $_FILES['attachments'];
    $updated_filename = $file['name']; 
    $updated_filerror = $file['error']; 
    $updated_filetmp = $file['tmp_name']; 
    $updated_target = "../files/".$updated_filename;
    move_uploaded_file($updated_filetmp,$updated_target);

    $updated_register_query="UPDATE notice1 SET notice_title= '$notice_title', notice_desc= '$notice_desc', eventid= '$event_id', attachments= '$updated_filename' where notice_id= $id";
            try{
                $updated_register_result = mysqli_query($conn,$updated_register_query);
